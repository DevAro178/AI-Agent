...
def main():
    try:
        ...
        if response.function_calls:
            ...
            print(function_call_parts)
            for function_call_part in function_call_parts:
                
                function_call_result=call_function(function_call_part,True if len(sys.argv) > 2 and sys.argv[2]=='--verbose' else False)
            
                if hasattr(function_call_result.parts[0].function_response,'response'):
                    if len(sys.argv) > 2 and sys.argv[2]=='--verbose':
                        print(f"-> {function_call_result.parts[0].function_response.response}")
                else:
                    print(f"-> {function_call_result.parts[0].function_response}")
                    raise Exception('Response missing in return statement')
